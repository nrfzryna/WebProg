...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    if (strpos($searchQuery, 'L') === 0 && strlen($searchQuery) === 4) {
        // Search by LeaveID
        $sql = "SELECT lh.*, gr.ManagerID, gr.ManagerName 
                FROM LeaveHistoryList lh
                LEFT JOIN Generatereport gr ON lh.LeaveID = gr.LeaveID
                WHERE lh.LeaveID = '$searchQuery' AND lh.Status IN ('Rejected', 'Approved')";
    } else {
        // Search by StaffID
        $sql = "SELECT lh.*, gr.ManagerID, gr.ManagerName 
                FROM LeaveHistoryList lh
                LEFT JOIN Generatereport gr ON lh.LeaveID = gr.LeaveID
                WHERE lh.StaffID = '$searchQuery' AND lh.Status IN ('Rejected', 'Approved')";
    }
} else {
